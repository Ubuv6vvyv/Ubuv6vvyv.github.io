...
for f in *.{jpg,jpeg,png,tif,tiff,webp}; do
    [ -e "$f" ] || continue
    val=$(magick "$f" -background white -alpha remove -thumbnail 300x -write "./thumbs/$f.jpg" -colorspace Gray -format "%[fx:mean]" info:)
...
montage "${thumbs[@]}" -geometry +10+10 -border 1 -label "%t" "montage_density.jpg"
