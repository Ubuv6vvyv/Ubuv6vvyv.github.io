javascript: !function() {
	...
	e.textContent = "Secret Finder Results";
	e.style.cssText = "margin: 0 0 15px 0; padding-right: 30px; font-size: 16px;";
	r.appendChild(e);
	...
	let i = {
		...
		async fetchWithTimeout(r, n = 5000) {
			let e = new AbortController(),
			S = setTimeout(() => e.abort(), n);
			try {
				let i = await fetch(r, {
					signal: e.signal, credentials: "omit"
				});
		...
		addResult(r, n, e) {
			...
			s.textContent = n;
			s.style.cssText = "margin: 0 0 5px 0; font-size: 14px; color: #444;";
			S.appendChild(s);

			let t = document.createElement("ul");
			t.style.cssText = "margin: 0; padding-left: 20px; font-size: 12px;";
			i.unique(e).forEach(r => {
	...
	let s = new class r {
		...
		async scanPage() {
			...
			r.textContent = "Scanning page...";
			this.container.appendChild(r);

			try {
				await this.scanContent(document.documentElement.outerHTML);

				let n = Array.from(document.getElementsByTagName("script")).filter(r => r.src).map(r => r.src);
				for (let e of n) {
					try {
						let S = await i.fetchWithTimeout(e);
						await this.scanContent(S);
					} catch (s) {
						console.warn(`Failed to fetch ${e}:`, s);
					}
				}
				...
				r.textContent = "Error during scan: " + t.message;
				r.style.color = "red";
			}
		}

		async scanContent(r) {
			for (let n of S.patterns.secrets) {
				for (let regex of n.regexArray) {
					let matches = r.match(regex);
					if (matches) {
						this.results.secrets.push(...matches.map(match => `${n.name}: ${match}`));
					}
				}
			}

			let i = r.match(/href=["']([^"']+)["']|src=["']([^"']+)["']/g);
			if (i) {
				let s = i.map(r => r.match(/["']([^"']+)["']/)[1]).filter(r => S.patterns.sensitiveFiles?.test(r));
				this.results.files.push(...s);
			}

			let t = r.match(/https?:\/\/[^\s<>"']+/g);
			if (t) this.results.urls.push(...t);
		}

		displayResults() {
			i.addResult(this.container, "Potential Secrets Found", this.results.secrets);
			i.addResult(this.container, "Sensitive Files", this.results.files);
			i.addResult(this.container, "URLs", this.results.urls);

			if (!this.results.secrets.length && !this.results.files.length) {
				let r = document.createElement("p");
				r.textContent = "No sensitive information found.";
				r.style.color = "#666";
				this.container.appendChild(r);
			}

			let n = document.createElement("button");
			n.textContent = "Export Results";
			n.style.cssText = `
			padding: 8px 15px;
			background: #007bff;
			color: white;
			...
			margin-top: 10px;
			`;
			n.onclick = () => this.exportResults();
			this.container.appendChild(n);
		}

		exportResults() {
			let r = new Blob([JSON.stringify(this.results, null, 2)], {
				type: "application/json"
			}),
			n = URL.createObjectURL(r),
			e = document.createElement("a");
			e.href = n;
			e.download = "secret-finder-results.json";
			e.click();
