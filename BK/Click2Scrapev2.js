(function() {
    ...
    function extractElementInfo(element) {
        ...
        return {
            text: isSVG ? '' : (useInnerHTML ? element.innerHTML : element.innerText || '').trim(),
    ...
    function createControlPanel() {
        ...
        panel.style.cssText = "position:fixed;top:20px;right:20px;background-color:#333;border:1px solid #555;padding:10px;z-index:10000;font-family:'Arial',sans-serif;box-shadow:0 0 10px rgba(0,0,0,0.5);color:#ddd;max-width:400px;width:100%;opacity:0.95;font-size:10px;";
        ...
        let clearLogButton = createButton("Clear Log");
        ...
        controls.innerHTML = `
            ...
            <br>
            <label for="useInnerHTML">
                <input type="checkbox" id="useInnerHTML" ${useInnerHTML ? 'checked' : ''}>
                Use innerHTML (includes HTML tags)
            </label>
        `;

        let output = document.createElement("div");
        output.style.cssText = "width:100%;height:200px;overflow-y:auto;border:1px solid #555;padding:5px;margin-top:10px;font-size:10px;background-color:#222;";

        panel.append(toggleButton, pauseResumeButton, startCrawlButton, cancelButton, clearSelectionsButton, downloadJSONButton, downloadCSVButton, downloadHTMLButton, collectLinksButton, closeButton, clearLogButton, controls, output);
        document.body.appendChild(panel);

        return {panel, toggleButton, pauseResumeButton, startCrawlButton, cancelButton, clearSelectionsButton, downloadJSONButton, downloadCSVButton, downloadHTMLButton, collectLinksButton, closeButton, clearLogButton, output};
    }
    ...
    let {panel, toggleButton, pauseResumeButton, startCrawlButton, cancelButton, clearSelectionsButton, downloadJSONButton, downloadCSVButton, downloadHTMLButton, collectLinksButton, closeButton, clearLogButton, output} = createControlPanel();
    controlPanel = panel;
    ...
    downloadCSVButton.addEventListener("click", function() {
        if (!scrapedData || scrapedData.length === 0) return void alert("No data to download. Please perform a crawl first.");
        let headers = Object.keys(scrapedData[0]).flatMap(key => [`${key} Text`, `${key} Href`, `${key} Src`, `${key} Selector`, `${key} TagName`]);
            ...
            <table>
                ...
                ${scrapedData.map(item => `
                    <tr>${Object.values(item).flatMap(value => [
                        ...
                        `<td>${value?.src ? `<a href="${value.src}" target="_blank"><img src="${value.src}" alt="Thumbnail" style="max-width:50px;max-height:50px;"></a>` : ""}</td>`,
            ...
            if (mutation.type === 'childList') {
                mutation.addedNodes.forEach((node) => {
                    if (node.nodeType === Node.ELEMENT_NODE) {
                        // Re-run selection on new elements
                        if (isSelecting && !isPaused) {
                            let selector = getElementSelector(node);
                            if (selectedElements.some(el => getElementSelector(el) === selector)) {
                                logOutput(output, "New element matches selected: " + selector);
                            }
                        }
                    }
                });
    ...
    // Toggle innerHTML/innerText
    document.getElementById("useInnerHTML").addEventListener("change", function() {
        useInnerHTML = this.checked;
        logOutput(output, `Switched to using ${useInnerHTML ? 'innerHTML' : 'innerText'} for text extraction.`);
    });

    console.log("Enhanced Multi-Select Scraper UI with selection control, CSP fallback, pagination support, link collection, and dynamic content handling added. Use the buttons to control the scraping process.");
})();
