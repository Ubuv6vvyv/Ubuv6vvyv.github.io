(function() {
    let selectedElements = [], isScrapingActive = false, scrapedData = [];
    function createUI() {
        const container = document.createElement('div');
        container.id = 'scraper-ui-container';
        container.style.cssText = `
            position: fixed; top: 20px; right: 20px; background-color: rgba(0, 0, 0, 0.8);
            color: #00FF00; border: 1px solid #444; padding: 8px; z-index: 10000;
            font-family: Arial, sans-serif; font-size: 14px; box-shadow: 0 0 10px rgba(0,0,0,0.5);
            border-radius: 5px; max-width: 90vw; max-height: 50vh; overflow-y: auto;
        `;
        const toggleButton = createButton('Start Selection'), startCrawlButton = createButton('Start Crawl'),
              cancelButton = createButton('Cancel'), downloadJSONButton = createButton('Download JSON'),
              downloadHTMLButton = createButton('Download HTML'), output = createOutput();
    ...
    function createButton(text) {
        ...
        button.style.cssText = `
            padding: 5px 8px; margin: 5px 0; width: 100%; background-color: #555;
            color: #00FF00; border: none; border-radius: 3px; cursor: pointer; font-size: 12px;
        `;
        return button;
    }
    function createOutput() {
        ...
        output.style.cssText = `
            width: 100%; height: 100px; overflow-y: auto; background-color: #222;
            color: #00FF00; border: 1px solid #444; padding: 5px; margin-top: 10px;
    ...
    function extractData(element) {
        const data = { text: element.innerText.trim(), href: element.href || element.querySelector('a')?.href || '', src: element.src || element.querySelector('img')?.src || '' };
        const attributes = {};
    ...
    function handleClick(event, output) {
        ...
        if (event.target.closest('#scraper-ui-container')) return;
    ...
    function startCrawl(output) {
        ...
        for (let i = 0; i < minLength; i++) {
            ...
            elementGroups.forEach((group, index) => {
                ...
                if (!scrapedData.some(d => JSON.stringify(d) === JSON.stringify(data))) {
    ...
    function downloadHTML() {
        ...
        <meta name="viewport" content="width=device-width, initial-scale=1.0"><title>Scraped Data</title>
        <style>body { font-family: Arial, sans-serif; padding: 20px; background-color: #f0f0f0; color: #333; }
        table { width: 100%; border-collapse: collapse; margin-top: 20px; }
        th, td { border: 1px solid #ccc; padding: 8px; text-align: left; }
        th { background-color: #555; color: white; }</style></head>
        <body><h1>Scraped Data</h1><table>
        <tr>${headers.map(header => `<th>${header}</th>`).join('')}</tr>
        ${scrapedData.map(item => `<tr>${headers.map(header => `
            <td><strong>Text:</strong> ${item[header].text || ''}<br>
    ...
    toggleButton.onclick = () => toggleScraping(toggleButton, output);
    startCrawlButton.onclick = () => startCrawl(output);
    cancelButton.onclick = () => cancelOperation(toggleButton, output);
