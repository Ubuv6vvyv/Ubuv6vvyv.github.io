...
(async function() {
    console.clear();
    const LOG_STYLES = {
        header: 'background: #222; color: #bada55; font-size: 14px; padding: 4px; font-weight: bold;',
        info: 'color: #00bcd4;',
        success: 'color: #4caf50; font-weight: bold;',
        warning: 'color: #ff9800;',
        danger: 'color: #f44336; font-weight: bold;',
        link: 'text-decoration: underline; color: #6ab0f3;'
    };
    ...
    function isStaticAsset(urlStr) {
        try {
            ...
            return CONFIG.ignoredExtensions.some(ext => path.endsWith(ext));
        } catch { return true; }
    }
    ...
    async function probeEndpoint(url) {
        try {
            ...
            return { url: url, valid: false, error: e.message };
        }
    }
    ...
    const scriptContents = await Promise.all(scripts.map(s => 
        fetch(s.src).then(r => r.text()).catch(() => "")
    ));
    combinedSource += "\n" + scriptContents.join("\n");
    ...
    rawUrls.forEach(u => {
        ...
        if (full && full.startsWith('http') && !isStaticAsset(full)) {
            uniqueUrls.add(full);
        }
    });
    ...
    candidates.forEach(u => {
        try {
            ...
            urlObj.searchParams.forEach((val, key) => {
                ...
                if (!paramMap[key].includes(u)) paramMap[key].push(u);
            });
        } catch(e){}
    });
    ...
    if (secrets.length > 0) {
        ...
        console.table(secrets.map(s => ({ secret: s.replace(/['"]/g, '') })));
        console.groupEnd();
    }
    ...
    if (jsonEndpoints.length > 0) {
        ...
        console.table(jsonEndpoints.map(e => ({
            Status: e.status,
            URL: e.url
        })));
        console.groupEnd();
    ...
    if (paramsFound.length > 0) {
        ...
        console.log("Parameters found in code. Useful for XSS/SQLi testing:");
        console.table(paramsFound.map(p => ({ Parameter: p, 'Example URL': paramMap[p][0] })));
        console.groupEnd();
    }
    ...
    return "Scan finished. Check the groups above for findings.";
